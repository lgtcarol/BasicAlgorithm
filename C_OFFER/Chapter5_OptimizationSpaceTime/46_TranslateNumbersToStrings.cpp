...
using namespace std;
/**
NOTE：动态规划可以分解出相同结构的子问题
      每个子问题的解个数结构相同，如本题：分为两种情况之和（注意区分什么是问题解结构和问题的解个数）
      
基础思路要有：自下而上给出解，与此对应需要每个子问题解个数的记录数组。 
...
int GetTranslationCount(const string& number)
{
    ...
    {
        count = 0;
         if(i < length - 1) //解的第一部分 
               count = counts[i + 1];
         else
               count = 1;

        if(i < length - 1) //解的第二部分，满足则为counts[i+2]，否则仅累计1？？ 
        {
