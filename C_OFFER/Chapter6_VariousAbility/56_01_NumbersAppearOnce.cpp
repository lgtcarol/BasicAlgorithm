...
void FindNumsAppearOnce(int data[], int length, int* num1, int* num2)
{
    ...
    int resultExclusiveOR = 0; //O(1)空间开销 
    for(int i = 0; i < length; ++i)
        resultExclusiveOR ^= data[i]; //正常int操作，内含bit规律 

    unsigned int indexOf1 = FindFirstBitIs1(resultExclusiveOR); //对一个整数的bit操作 

    *num1 = *num2 = 0;
    for(int j = 0; j < length; ++j)
    {
        if(IsBit1(data[j], indexOf1)) //判断某整数的一个bit位 
            *num1 ^= data[j];
        else
            *num2 ^= data[j];
    }
}
//  Attention:如何找出一个int数的最右端bit取1的index 
// 找到num从右边数起第一个是1的位
unsigned int FindFirstBitIs1(int num)
{
    int indexBit = 0;
